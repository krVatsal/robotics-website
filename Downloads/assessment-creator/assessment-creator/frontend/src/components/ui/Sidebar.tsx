...
const getNavItems = (assignmentCount: number) => [
  { label: 'Home', icon: Home, href: '/', active: false, badge: 0 },
  { label: 'My Groups', icon: Users, href: '/groups', active: false, badge: 0 },
  { label: 'Assignments', icon: FileText, href: '/', active: true, badge: assignmentCount },
  { label: "AI Teacher's Toolkit", icon: BookOpen, href: '/toolkit', active: false, badge: 0 },
  { label: 'My Library', icon: FolderOpen, href: '/library', active: false, badge: 0 },
];
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  size={20}
                  className={item.active ? 'text-text-primary' : 'text-text-secondary'}
                ...
                {item.badge > 0 && (
                  <span className="bg-btn-orange text-white text-xs font-semibold px-2 py-0.5 rounded-lg min-w-[24px] text-center">
                    {item.badge}
                  </span>
                )}
              </Link>
